...
class MerklePatriciaTrie:
    ...
    def get_proof(self, encoded_key):
        """
        生成指定键的证明，并返回证明的 VO 大小。
        ...
        返回：
        tuple:
            - list of bytes: 证明所需的节点列表（RLP 编码）。
            - int: VO 的总大小，包含路径、兄弟节点及 proof 大小。
        ...
        # 计算路径的大小
        vo_size += sys.getsizeof(path)

        # 调用 _get_proof 函数，生成 proof 并计算 VO 大小
        self._get_proof(self._root, path, proof, vo_size)

        return proof, vo_size

    def _get_proof(self, node_ref, path, proof, vo_size):
        node = self._get_node(node_ref)
        ...
        # 计算 proof 中每个节点的大小
        proof.append(encoded_node)
        vo_size += sys.getsizeof(encoded_node)
        ...
        if isinstance(node, Node.Leaf):
            ...
            if path.starts_with(node.path):
                remaining_path = NibblePath(path._data, path._offset)
                remaining_path.consume(len(node.path))
                # 计算路径中的节点大小
                vo_size += sys.getsizeof(remaining_path)
                self._get_proof(node.next_ref, remaining_path, proof, vo_size)
            else:
                raise KeyError("Key not found in trie")
            ...
            if len(path) == 0:
                if node.data:
                    ...
                    raise KeyError("Key not found in trie")
            idx = path.at(0)
            next_ref = node.branches[idx]
            if not next_ref:
                raise KeyError("Key not found in trie")

            # 计算兄弟节点大小（除了选择的分支之外的其他分支）
            for i, branch in enumerate(node.branches):
                if branch and i != idx:
                    vo_size += sys.getsizeof(branch)

            remaining_path = NibblePath(path._data, path._offset)
            remaining_path.consume(1)
            self._get_proof(next_ref, remaining_path, proof, vo_size)
        else:
            raise KeyError("Unknown node type")

    @staticmethod
    def verify_proof(root_hash, encoded_key, proof):
        from .nibble_path import NibblePath
        from .node import Node
        from .hash import keccak_hash
        import rlp
        ...
        try:
            while idx < len(proof):
                encoded_node = proof[idx]
                node_hash = keccak_hash(encoded_node)
                ...
                if isinstance(node, Node.Leaf):
                    ...
                    if path.starts_with(node.path):
                        ...
                        current_hash = keccak_hash(next_ref) if len(next_ref) < 32 else next_ref
                    else:
                        return False, None
                    ...
                    if len(path) == 0:
                        if node.data:
                            ...
                            return False, None
                    idx_nibble = path.at(0)
                    path.consume(1)
                    next_ref = node.branches[idx_nibble]
                    if not next_ref:
                        return False, None
                    current_hash = keccak_hash(next_ref) if len(next_ref) < 32 else next_ref
                else:
            ...
            print("Exception during proof verification:", e)
            return False, None
